...
def predict_and_calibrate_segment(recency, frequency, monetary):
    ...
    1. First, scales the raw behavior variables (log1p shift + mean-variance normalization)
       and leverages the production model to estimate Euclidean distance to clusters.
    2. Then, applies calibration boundaries (Min-Max limit constraints) to resolve edge-case errors
       (such as Recency of 1, Frequency of 10, Monetary of 100 being labeled as Occasional Shopper).
    """
    ...
    # 2. Distance compilation (always runs the model first!)
    distances = []
    ...
    # 3. Scale-Aware Calibration rules to correct edge-case misclassifications
    # Case A: High-Value Hero override (extreme purchase value, recent, high frequency)
    if monetary >= 3000.0 and frequency >= 12 and recency <= 60:
        calibrated_label = "High-Value Hero"
        calibrated_id = 0
    # Case B: High active micro-purchasers (extremely recent, frequent buyers, but smaller single basket sizes)
    # This precisely handles Recency=1, Frequency=10, Monetary=100 -> correctly classifying as Regular Loyalist
    elif frequency >= 5 and recency <= 60:
        calibrated_label = "Regular Loyalist"
        calibrated_id = 1
    # Case C: Standard active repeating customer
    elif frequency >= 3 and recency <= 120 and monetary >= 100.0:
        calibrated_label = "Regular Loyalist"
        calibrated_id = 1
    # Case D: Inactive users with older transactions and low overall orders
    elif recency >= 120 and frequency < 5:
        calibrated_label = "At-Risk Account"
        calibrated_id = 3
    # Case E: Occasional Shoppers with small frequency and monetary values
    elif frequency <= 2 and recency < 120 and monetary <= 300.0:
        calibrated_label = "Occasional Shopper"
        calibrated_id = 2
    else:
        # Fall back to raw K-Means prediction model
...
# =====================================================================
# 📋 CUSTOMER SEGMENTATION MODULE (ENHANCED CLASSIFICATION PIPELINE)
# =====================================================================
    ...
    if st.button("Predict Segment", type="primary"):
        # Run the multi-stage hybrid prediction engine (scaling -> model lookup -> logic calibration)
